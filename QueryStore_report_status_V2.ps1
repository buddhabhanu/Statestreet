...
$htmlFilePath = "\\VBOXSVR\ORACLE_Share\PS_SQL_Health_report\reports\SQLQueryStore_Status.html"
...
$query = "
IF OBJECT_ID('master..server') IS NULL
Create table master..server (ServerName varchar(100), Database_Name varchar(100),QS_Status varchar(100))
insert into master..server
SELECT @@servername,name, case when (is_query_store_on=0) then 'Disabled' else 'Enabled' end 
FROM sys.databases
WHERE database_id>4
select * from master..server
"
...
$html = "<html><body>"
$html += "<div><img align=right  style=height: 50px; width: 50px/><font face=Verdana size=4 color=#3399ff><H2><bold>MSSQL Server Query Store Validation Report</bold></H2></font></div>                                  
...
$query1 = "
Drop table master..server
"
$results.Version = Invoke-SqlQuery -connectionString $connectionString -query $query1
...
$subject = "Summary for Query Store Status Check on SQL Server "+ $server +" is " + "$results.Summary.name"
